...
const Plinko: React.FC<PlinkoProps> = ({ onBet, onWin }) => {
  ...
  const COLORS = {
    background: '#0f212e',
    peg: '#e2e8f0',
    pegActive: '#00FFFF', // Brand Cyan
    ball: '#00FFFF',      // Brand Cyan
    text: '#ffffff',
  };
  ...
  const multipliers = useMemo(() => {
    ...
    const profiles = {
      Low: { center: 0.5, edge: 5, curve: 3.0 }, 
      Medium: { center: 0.2, edge: 15, curve: 4.5 }, 
      High: { center: 0.0, edge: 100, curve: 6.0 } 
    };
  ...
  const getBucketColor = (mult: number) => {
    if (mult >= 50) return '#CE2029'; // Brand Red
    if (mult >= 10) return '#FFA500'; // Brand Orange
    if (mult >= 3) return '#FFFF33';  // Brand Yellow
    if (mult >= 1.5) return '#00FFFF'; // Brand Cyan
    return '#1a2c38'; // Dark Slate
  };
  ...
  const update = (time: number) => {
    ...
    const gap = Math.min(width / (rows + 2), availableH / (rows + 2)); // Larger Board
    ...
    const bucketH = Math.max(24, gap * 0.6); // Scaling bucket height
    ...
    multipliers.forEach((mult, i) => {
       ...
       // Text
       ctx.fillStyle = mult >= 10 || mult < 1 ? '#fff' : '#000'; // Contrast text
       // Use Anton font
       ctx.font = `400 ${Math.min(14, gap * 0.35)}px "Anton"`;
       ctx.textAlign = 'center';
       ctx.textBaseline = 'middle';
       ctx.fillText(`${mult}x`, bx, currentBucketY + bucketH / 2 + 1);
    });
    ...
    ballsRef.current.forEach((ball) => {
       ...
       if (ball.isInBucket) {
               ...
               for (let i = 0; i < pegsInRow; i++) {
                   ...
                   if (dist < minDist) {
                       ...
                       if (checkRow < ball.path.length) {
                           const desiredDirection = ball.path[checkRow] === 1 ? 1 : -1; 
                           const jitter = (Math.random() - 0.5) * 50;
                           ball.vx += (desiredDirection * 150 + jitter) * dt * 5; 
                           
                           spawnParticles(px, py, 'rgba(0, 255, 255, 0.3)', 1); // Cyan sparks
                       }
                   }
               }
           }
           ...
           if (ball.y > hitY) {
               ...
               if (mult > 0) onWin(ball.wager * mult);
    ...
    <div className="flex flex-col lg:flex-row gap-6 w-full max-w-7xl mx-auto p-4 h-[calc(100vh-100px)] min-h-[600px]">
      ...
      <div className="w-full lg:w-80 flex-shrink-0 bg-card p-6 rounded-xl flex flex-col gap-6 shadow-xl border border-slate-800/50 overflow-y-auto">
        <div className="flex items-center justify-between text-white mb-2">
           <div className="flex items-center gap-2">
             <Zap size={24} className="fill-current text-brand-cyan" />
             <span className="text-2xl font-heading uppercase tracking-wider">Plinko</span>
           </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-bold text-gray-400 uppercase">
            ...
            <span className="font-sub text-lg">${parseFloat(betAmount || '0').toFixed(2)}</span>
          </div>
          <div className="relative group">
             ...
             <input 
               ...
               className="w-full bg-background border border-slate-700 rounded-lg py-3 pl-7 pr-3 text-white font-sub text-lg focus:border-brand-cyan outline-none transition-colors"
             />
          </div>
             ...
             {['Low', 'Medium', 'High'].map(r => (
               <button
                 ...
                 className={`py-2 rounded text-xs font-bold transition-all 
                   ${risk === r ? 'bg-card text-brand-cyan shadow shadow-black/20' : 'text-gray-500 hover:text-white'}
        ...
        <div>
           <div className="flex justify-between mb-3">
              ...
              <span className="text-xs font-bold text-white bg-slate-800 px-2 py-0.5 rounded flex items-center gap-1 font-sub">
                 {rows}
           ...
           <input 
             ...
             disabled={isDropping}
             className={`w-full accent-brand-cyan h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer ${isDropping ? 'opacity-50 cursor-not-allowed' : ''}`}
           />
        ...
        <button 
            onClick={dropBall}
            className="w-full bg-gradient-to-r from-brand-red to-brand-orange hover:brightness-110 text-white font-heading text-2xl py-4 rounded-lg shadow-[0_0_20px_rgba(206,32,41,0.3)] transition-all active:scale-95 mt-auto flex items-center justify-center gap-2"
          >
            <span>DROP BALL</span>
        </button>
      </div>
      ...
      <div ref={containerRef} className="flex-1 bg-[#0f212e] rounded-xl border border-slate-800 relative overflow-hidden shadow-2xl flex items-center justify-center">
         ...
         {lastMulti && (
            ...
            >
              <div className="bg-slate-800/90 backdrop-blur border border-slate-600 px-6 py-4 rounded-xl shadow-2xl flex flex-col items-center gap-1">
                 <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Win</span>
                 <div className={`text-4xl font-sub ${lastMulti >= 1 ? 'text-brand-yellow' : 'text-gray-400'}`}>
                   {lastMulti}x
                 </div>
              </div>
            </div>
